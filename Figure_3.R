...
for (i in 1:2){
  for (j in feature.range){
    group.1 = as.matrix(bm.sort[(bm.sort$Vax_timing_bin == 3 | bm.sort$Vax_timing_bin == 4 | bm.sort$Vax_timing_bin == 5) &  bm.sort$Delta_weeks_bin==i,j])
    group.2 = as.matrix(bm.sort[bm.sort$Vax_timing_bin == 1 & bm.sort$Delta_weeks_bin==i,j])
    
    stat.result = wilcox.test(group.1,group.2)
    my.pvals[i,j-3] = stat.result$p.value
    
    effect_size = cohen.d(log10(group.1),log10(group.2), hedges.correction = T)
    cohens.d[i,j-3] = effect_size[["estimate"]]
...
# plot heatmap
pheatmap(cohens.d,color=myColor,cluster_rows=F,cluster_cols=F,gaps_col=c(seq(from=21,to=ncol(cohens.d),by=21),ncol(cohens.d)-3),breaks=myBreaks,fontsize_col=4,  number_color='black',fontsize_number=14)
...
vax.t1 = bm[(bm$Vax_timing_bin == 3 | bm$Vax_timing_bin == 4 | bm$Vax_timing_bin == 5) & bm$Delta_weeks_bin==1,] %>% dplyr::select(contains(myAntigen)) %>% log10()
vax.t1 = pivot_longer(vax.t1,cols=colnames(vax.t1),names_to='Ag',values_to='MFI')
vax.t1$Fc = sub("\\_.*", "", vax.t1$Ag)
vax.t1$group = 'Vax_2wk'

vax.t2 = bm[(bm$Vax_timing_bin == 3 | bm$Vax_timing_bin == 4 | bm$Vax_timing_bin == 5) & bm$Delta_weeks_bin==2,] %>% dplyr::select(contains(myAntigen)) %>% log10()
vax.t2 = pivot_longer(vax.t2,cols=colnames(vax.t2),names_to='Ag',values_to='MFI')
vax.t2$Fc = sub("\\_.*", "", vax.t2$Ag)
vax.t2$group = 'Vax_6wk'

novax.t1 = bm[(bm$Vax_timing_bin == 1) & bm$Delta_weeks_bin==1,] %>% dplyr::select(contains(myAntigen)) %>% log10()
...
novax.t2 = bm[(bm$Vax_timing_bin == 1) & bm$Delta_weeks_bin==2,] %>% dplyr::select(contains(myAntigen)) %>% log10()
...
ggplot(plot.data, aes(x = group, y =MFI, color = as.factor(group), group = as.factor(group))) + facet_wrap(. ~ Fc, nrow = 2, scales = "free") + 
